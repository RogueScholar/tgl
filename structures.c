/* SPDX-FileCopyrightText: © 2013-2015 Vitaly Valtman <mail@vysheng.ru>
 * SPDX-FileCopyrightText: © 2014 Paul Eipper <paul@nkey.com.br>
 * SPDX-FileCopyrightText: © 2015 Matthias Jentsch <mtthsjntsch@gmail.com>
 * SPDX-FileCopyrightText: © 2015 Eion Robb <eion@robbmob.com>
 * SPDX-FileCopyrightText: © 2015-2016 Vaibhav Verma <vaibhav2614@gmail.com>
 * SPDX-FileCopyrightText: © 2015-2019 Ben Wiederhake <BenWiederhake@gmx.de>
 * SPDX-FileCopyrightText: © 2016 Garemyah Johnson
 * SPDX-FileCopyrightText: © 2017 LucentW
 * SPDX-FileCopyrightText: © 2017 Giuseppe Marino
 * SPDX-FileCopyrightText: © 2019 Rafal Wieczorek <kenorb@gmail.com>
 * SPDX-FileCopyrightText: © 2019 Oleg78
 * SPDX-FileCopyrightText: © 2019 vuori <vuori@notcom.org>
...
tglf_fetch_typing(struct tl_ds_send_message_action *DS_SMA) {
  if (!DS_SMA) {
    return 0;
  }
  switch (DS_SMA->magic) {
...
/*enum tgl_typing_status tglf_fetch_typing (void) {
    struct tl_ds_send_message_action *DS_SMA = fetch_ds_type_send_message_action
    (TYPE_TO_PARAM (send_message_action)); enum tgl_typing_status res =
    tglf_fetch_typing (DS_SMA); free_ds_type_send_message_action (DS_SMA,
    TYPE_TO_PARAM (send_message_action)); return res;
  }
 */
...
int tglf_fetch_file_location(struct tgl_state *TLS,
                             struct tgl_file_location *loc,
                             struct tl_ds_file_location *DS_FL) {
  if (!DS_FL) {
    return 0;
  }
...
int tglf_fetch_user_status(struct tgl_state *TLS, struct tgl_user_status *S,
                           struct tgl_user *U,
                           struct tl_ds_user_status *DS_US) {
  if (!DS_US) {
    return 0;
  }
  switch (DS_US->magic) {
...
struct tgl_user *tglf_fetch_alloc_user(struct tgl_state *TLS,
  ...
  bl_do_user(TLS, tgl_get_peer_id(U->id), DS_U->access_hash,
             DS_STR(DS_U->first_name), DS_STR(DS_U->last_name),
             DS_STR(DS_U->phone), DS_STR(DS_U->username), NULL, DS_U->photo,
...
struct tgl_user *tglf_fetch_alloc_user_full(struct tgl_state *TLS,
  ...
  bl_do_user(TLS, tgl_get_peer_id(U->id), NULL, NULL, 0, NULL, 0, NULL, 0, NULL,
             0, DS_UF->profile_photo, NULL, NULL, NULL, DS_UF->bot_info, flags);
  return U;
}
...
tglf_fetch_alloc_encrypted_chat(struct tgl_state *TLS,
                                struct tl_ds_encrypted_chat *DS_EC) {
  if (!DS_EC) {
    return NULL;
  }
  ...
  if (DS_EC->magic == CODE_encrypted_chat_discarded) {
    if (new) {
      vlogprintf(
          E_WARNING,
          "Unknown chat in deleted state. May be we forgot something...\n");
      return U;
    }
    bl_do_peer_delete(TLS, U->id);
  ...
  if (new) {
    ...
    int user_id = DS_LVAL(DS_EC->participant_id) + DS_LVAL(DS_EC->admin_id) -
                  tgl_get_peer_id(TLS->our_id);
    int r = sc_request;
    bl_do_encr_chat(TLS, tgl_get_peer_id(U->id), DS_EC->access_hash,
                    DS_EC->date, DS_EC->admin_id, &user_id, NULL, (void *)g_key,
                    NULL, &r, NULL, NULL, NULL, NULL, NULL, NULL,
                    TGLECF_CREATE | TGLECF_CREATED, NULL, 0);
  } else {
    if (DS_EC->magic == CODE_encrypted_chat_waiting) {
      int r = sc_waiting;
      bl_do_encr_chat(TLS, tgl_get_peer_id(U->id), DS_EC->access_hash,
                      DS_EC->date, NULL, NULL, NULL, NULL, NULL, &r, NULL, NULL,
                      NULL, NULL, NULL, NULL, TGL_FLAGS_UNCHANGED, NULL, 0);
    ...
    int r = sc_ok;
    bl_do_encr_chat(TLS, tgl_get_peer_id(U->id), DS_EC->access_hash,
                    DS_EC->date, NULL, NULL, NULL, g_key, NULL, &r, NULL, NULL,
                    NULL, NULL, NULL, DS_EC->key_fingerprint,
                    TGL_FLAGS_UNCHANGED, NULL, 0);
  }
  return U;
}

struct tgl_chat *tglf_fetch_alloc_chat(struct tgl_state *TLS,
                                       struct tl_ds_chat *DS_C) {
  if (!DS_C) {
    return NULL;
  }
  ...
  bl_do_chat(TLS, tgl_get_peer_id(C->id), DS_STR(DS_C->title),
             DS_C->participants_count, DS_C->date, NULL, NULL, DS_C->photo,
             NULL, NULL, NULL, NULL, flags);
  return C;
}

struct tgl_chat *
tglf_fetch_alloc_chat_full(struct tgl_state *TLS,
                           struct tl_ds_messages_chat_full *DS_MCF) {
  if (!DS_MCF) {
    return NULL;
  }
  ...
  if (DS_CF->bot_info) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (P && (P->flags & TGLCF_CREATED)) {
        bl_do_user(TLS, tgl_get_peer_id(P->id), NULL, NULL, 0, NULL, 0, NULL, 0,
                   NULL, 0, NULL, NULL, NULL, NULL, DS_BI, TGL_FLAGS_UNCHANGED);
      }
    }
  }
  ...
  struct tgl_chat *C = (void *)tgl_peer_get(TLS, chat_id);
  assert(C);
  bl_do_chat(TLS, tgl_get_peer_id(C->id), NULL, 0, NULL, NULL,
             DS_CF->participants->version,
             (struct tl_ds_vector *)DS_CF->participants->participants, NULL,
             DS_CF->chat_photo, NULL,
             // DS_CF->participants->admin_id,
             NULL, NULL, C->flags & 0xffff);
  return C;
}

struct tgl_channel *tglf_fetch_alloc_channel(struct tgl_state *TLS,
                                             struct tl_ds_chat *DS_C) {
  if (!DS_C) {
    return NULL;
  }
  ...
  bl_do_channel(TLS, tgl_get_peer_id(C->id), DS_C->access_hash, DS_C->date,
                DS_STR(DS_C->title), DS_STR(DS_C->username), DS_C->photo, NULL,
                NULL, NULL, 0, NULL, NULL, NULL, NULL, flags);
  return C;
}

struct tgl_channel *
tglf_fetch_alloc_channel_full(struct tgl_state *TLS,
                              struct tl_ds_messages_chat_full *DS_MCF) {
  if (!DS_MCF) {
    return NULL;
  }
  ...
  struct tgl_channel *C = (void *)tgl_peer_get(TLS, chat_id);
  assert(C);
  bl_do_channel(TLS, tgl_get_peer_id(C->id), NULL, NULL, NULL, 0, NULL, 0, NULL,
                DS_CF->chat_photo, NULL, DS_STR(DS_CF->about),
                DS_CF->participants_count, DS_CF->admins_count,
                DS_CF->kicked_count, DS_CF->read_inbox_max_id,
                TGL_FLAGS_UNCHANGED);
  return C;
}

void tglf_fetch_photo_size(struct tgl_state *TLS, struct tgl_photo_size *S,
                           struct tl_ds_photo_size *DS_PS) {
  memset(S, 0, sizeof(*S));
...
void tglf_fetch_geo(struct tgl_state *TLS, struct tgl_geo *G,
                    struct tl_ds_geo_point *DS_GP) {
  G->longitude = DS_LVAL(DS_GP->longitude);
  G->latitude = DS_LVAL(DS_GP->latitude);
}

struct tgl_photo *tglf_fetch_alloc_photo(struct tgl_state *TLS,
                                         struct tl_ds_photo *DS_P) {
  if (!DS_P) {
    return NULL;
  }
...
struct tgl_document *tglf_fetch_alloc_video(struct tgl_state *TLS,
                                            struct tl_ds_video *DS_V) {
  if (!DS_V) {
    return NULL;
  }
...
struct tgl_document *tglf_fetch_alloc_audio(struct tgl_state *TLS,
                                            struct tl_ds_audio *DS_A) {
  if (!DS_A) {
    return NULL;
  }
...
void tglf_fetch_document_attribute(struct tgl_state *TLS,
                                   struct tgl_document *D,
                                   struct tl_ds_document_attribute *DS_DA) {
  switch (DS_DA->magic) {
...
struct tgl_document *tglf_fetch_alloc_document(struct tgl_state *TLS,
                                               struct tl_ds_document *DS_D) {
  if (!DS_D) {
    return NULL;
  }
...
struct tgl_webpage *tglf_fetch_alloc_webpage(struct tgl_state *TLS,
                                             struct tl_ds_web_page *DS_W) {
  if (!DS_W) {
    return NULL;
  }
...
void tglf_fetch_message_action(struct tgl_state *TLS,
                               struct tgl_message_action *M,
                               struct tl_ds_message_action *DS_MA) {
  if (!DS_MA) {
    return;
  }
  memset(M, 0, sizeof(*M));
...
struct tgl_message *tglf_fetch_alloc_message_short(struct tgl_state *TLS,
  ...
  bl_do_edit_message(
      TLS, &msg_id, (f & 2) ? &our_id : &peer_id, (f & 2) ? &peer_id : &our_id,
      DS_U->fwd_from_id ? &fwd_from_id : NULL, DS_U->fwd_date, DS_U->date,
      DS_STR(DS_U->message), &A, NULL, DS_U->reply_to_msg_id, NULL,
      (void *)DS_U->entities, flags);
  return M;
}

struct tgl_message *
tglf_fetch_alloc_message_short_chat(struct tgl_state *TLS,
                                    struct tl_ds_updates *DS_U) {
  ...
  bl_do_edit_message(TLS, &msg_id, &from_id, &to_id,
                     DS_U->fwd_from_id ? &fwd_from_id : NULL, DS_U->fwd_date,
                     DS_U->date, DS_STR(DS_U->message), &A, NULL,
                     DS_U->reply_to_msg_id, NULL, NULL, flags);
  return M;
}

void tglf_fetch_message_media(struct tgl_state *TLS,
                              struct tgl_message_media *M,
                              struct tl_ds_message_media *DS_MM) {
  if (!DS_MM) {
    return;
  }
  memset(M, 0, sizeof(*M));
...
void tglf_fetch_message_media_encrypted(
    struct tgl_state *TLS, struct tgl_message_media *M,
    struct tl_ds_decrypted_message_media *DS_DMM) {
  if (!DS_DMM) {
    return;
  }
  memset(M, 0, sizeof(*M));
...
void tglf_fetch_message_action_encrypted(
    struct tgl_state *TLS, struct tgl_message_action *M,
    struct tl_ds_decrypted_message_action *DS_DMA) {
  if (!DS_DMA) {
    return;
  }
  switch (DS_DMA->magic) {
...
void tglf_fetch_message_entity(struct tgl_state *TLS,
                               struct tgl_message_entity *E,
                               struct tl_ds_message_entity *DS_ME) {
...
void tglf_fetch_message_entities(struct tgl_state *TLS, struct tgl_message *M,
                                 struct tl_ds_vector *DS) {
...
struct tgl_message *tglf_fetch_alloc_message(struct tgl_state *TLS,
  ...
  if (new) {
    ...
    bl_do_edit_message(TLS, &msg_id, DS_M->from_id ? &from_id : NULL, &to_id,
                       DS_M->fwd_from_id ? &fwd_from_id : NULL, DS_M->fwd_date,
                       DS_M->date, DS_STR(DS_M->message), DS_M->media,
                       DS_M->action, DS_M->reply_to_msg_id, DS_M->reply_markup,
                       (void *)DS_M->entities,
                       flags | TGLMF_CREATE | TGLMF_CREATED);
  }
  return M;
...
static int decrypt_encrypted_message(struct tgl_secret_chat *E) {
  ...
  int *e_key =
      E->exchange_state != tgl_sce_committed ? E->key : E->exchange_key;
  ...
  TGLC_aes_ige_encrypt((void *)decr_ptr, (void *)decr_ptr,
                       4 * (decr_end - decr_ptr), &aes_key, iv, 0);
...
struct tgl_message *
tglf_fetch_encrypted_message(struct tgl_state *TLS,
                             struct tl_ds_encrypted_message *DS_EM) {
  if (!DS_EM) {
    return NULL;
  }
  ...
  tgl_message_id_t msg_id =
      tgl_peer_id_to_msg_id(P->id, DS_LVAL(DS_EM->random_id));
  struct tgl_message *M = tgl_message_get(TLS, &msg_id);
  if (!M) {
    M = talloc0(sizeof(*M));
    M->permanent_id = msg_id;
  ...
  if (P->encr_chat.exchange_state == tgl_sce_committed &&
      P->encr_chat.key_fingerprint == *(long long *)decr_ptr) {
    tgl_do_confirm_exchange(TLS, (void *)P, 0);
    assert(P->encr_chat.exchange_state == tgl_sce_none);
  }
  long long key_fingerprint = P->encr_chat.exchange_state != tgl_sce_committed
                                  ? P->encr_chat.key_fingerprint
                                  : P->encr_chat.exchange_key_fingerprint;
  ...
  assert(fetch_int() == ll);
  if (skip_type_decrypted_message_layer(
          TYPE_TO_PARAM(decrypted_message_layer)) < 0 ||
      in_ptr != in_end) {
  ...
  in_ptr = decr_ptr;
  assert(fetch_int() == ll);
  struct tl_ds_decrypted_message_layer *DS_DML =
      fetch_ds_type_decrypted_message_layer(
          TYPE_TO_PARAM(decrypted_message_layer));
  assert(DS_DML);
  in_ptr = save_in_ptr;
  in_end = save_in_end;
  // bl_do_encr_chat_set_layer (TLS, (void *)P, DS_LVAL (DS_DML->layer));
  bl_do_encr_chat(TLS, tgl_get_peer_id(P->id), NULL, NULL, NULL, NULL, NULL,
                  NULL, NULL, NULL, NULL, DS_DML->layer, NULL, NULL, NULL, NULL,
                  TGL_FLAGS_UNCHANGED, NULL, 0);
  ...
  if (in_seq_no / 2 != P->encr_chat.in_seq_no) {
    ...
    free_ds_type_decrypted_message_layer(
        DS_DML, TYPE_TO_PARAM(decrypted_message_layer));
    return M;
  }
  if ((in_seq_no & 1) !=
          1 - (P->encr_chat.admin_id == tgl_get_peer_id(TLS->our_id)) ||
      (out_seq_no & 1) !=
          (P->encr_chat.admin_id == tgl_get_peer_id(TLS->our_id))) {
  ...
  if (out_seq_no / 2 > P->encr_chat.out_seq_no) {
    vlogprintf(E_WARNING,
               "In seq no is bigger than our's out seq no (out_seq_no = %d, "
               "our_out_seq_no = %d). Drop\n",
  ...
  if (out_seq_no / 2 < P->encr_chat.last_in_seq_no) {
    vlogprintf(E_WARNING,
               "Clients in_seq_no decreased (out_seq_no = %d, last_out_seq_no "
               "= %d). Drop\n",
  ...
  if (M->permanent_id.id != DS_LVAL(DS_DM->random_id)) {
    vlogprintf(E_ERROR,
               "Incorrect message: id = %" INT64_PRINTF_MODIFIER
               "d, new_id = %" INT64_PRINTF_MODIFIER "d\n",
  ...
  bl_do_edit_message_encr(TLS, &M->permanent_id, &from_id, &P->id, DS_EM->date,
                          DS_STR(DS_DM->message), DS_DM->media, DS_DM->action,
                          DS_EM->file,
                          TGLMF_CREATE | TGLMF_CREATED | TGLMF_ENCRYPTED);
  if (in_seq_no >= 0 && out_seq_no >= 0) {
    // bl_do_encr_chat_update_seq (TLS, (void *)P, in_seq_no / 2 + 1,
    // out_seq_no / 2);
    in_seq_no = in_seq_no / 2 + 1;
    out_seq_no = out_seq_no / 2;
    bl_do_encr_chat(TLS, tgl_get_peer_id(P->id), NULL, NULL, NULL, NULL, NULL,
                    NULL, NULL, NULL, NULL, NULL, &in_seq_no, &out_seq_no, NULL,
                    NULL, TGL_FLAGS_UNCHANGED, NULL, 0);
    assert(P->encr_chat.in_seq_no == in_seq_no);
  }
  free_ds_type_decrypted_message_layer(DS_DML,
                                       TYPE_TO_PARAM(decrypted_message_layer));
  return M;
}

void tglf_fetch_encrypted_message_file(struct tgl_state *TLS,
                                       struct tgl_message_media *M,
                                       struct tl_ds_encrypted_file *DS_EF) {
...
static int id_cmp(struct tgl_message *M1, struct tgl_message *M2) {
  if (M1->permanent_id.peer_type < M2->permanent_id.peer_type) {
    return -1;
  }
  if (M1->permanent_id.peer_type > M2->permanent_id.peer_type) {
    return 1;
  }
  if (M1->permanent_id.peer_id < M2->permanent_id.peer_id) {
    return -1;
  }
  if (M1->permanent_id.peer_id > M2->permanent_id.peer_id) {
    return 1;
  }
  if (M1->permanent_id.id < M2->permanent_id.id) {
    return -1;
  }
  if (M1->permanent_id.id > M2->permanent_id.id) {
    return 1;
...
static void increase_peer_size(struct tgl_state *TLS) {
  if (TLS->peer_num == TLS->peer_size) {
    ...
    if (old_size) {
      TLS->Peers = trealloc(TLS->Peers, old_size * sizeof(void *),
...
struct tgl_message *
tglf_fetch_alloc_encrypted_message(struct tgl_state *TLS,
                                   struct tl_ds_encrypted_message *DS_EM) {
  ...
  if (M->flags & TGLMF_CREATED) {
    ...
    if (M->action.type == tgl_message_action_request_key) {
      if (E->exchange_state == tgl_sce_none ||
          (E->exchange_state == tgl_sce_requested &&
           E->exchange_id > M->action.exchange_id)) {
        tgl_do_accept_exchange(TLS, E, M->action.exchange_id, M->action.g_a);
      } else {
        vlogprintf(E_WARNING,
                   "Exchange: Incorrect state (received request, state = %d)\n",
                   E->exchange_state);
      }
    }
    if (M->action.type == tgl_message_action_accept_key) {
      if (E->exchange_state == tgl_sce_requested &&
          E->exchange_id == M->action.exchange_id) {
        tgl_do_commit_exchange(TLS, E, M->action.g_a);
      } else {
        vlogprintf(E_WARNING,
                   "Exchange: Incorrect state (received accept, state = %d)\n",
                   E->exchange_state);
      }
    }
    if (M->action.type == tgl_message_action_commit_key) {
      if (E->exchange_state == tgl_sce_accepted &&
          E->exchange_id == M->action.exchange_id) {
        tgl_do_confirm_exchange(TLS, E, 1);
      } else {
        vlogprintf(E_WARNING,
                   "Exchange: Incorrect state (received commit, state = %d)\n",
                   E->exchange_state);
      }
    }
    if (M->action.type == tgl_message_action_abort_key) {
      if (E->exchange_state != tgl_sce_none &&
        ...
        vlogprintf(E_WARNING,
                   "Exchange: Incorrect state (received abort, state = %d)\n",
                   E->exchange_state);
      }
    }
    if (M->action.type == tgl_message_action_notify_layer) {
      bl_do_encr_chat(TLS, tgl_get_peer_id(E->id), NULL, NULL, NULL, NULL, NULL,
                      NULL, NULL, NULL, NULL, &M->action.layer, NULL, NULL,
                      NULL, NULL, TGL_FLAGS_UNCHANGED, NULL, 0);
    }
    if (M->action.type == tgl_message_action_set_message_ttl) {
      // bl_do_encr_chat_set_ttl (TLS, E, M->action.ttl);
      bl_do_encr_chat(TLS, tgl_get_peer_id(E->id), NULL, NULL, NULL, NULL, NULL,
                      NULL, NULL, NULL, &M->action.ttl, NULL, NULL, NULL, NULL,
                      NULL, TGL_FLAGS_UNCHANGED, NULL, 0);
    }
  }
  return M;
...
struct tgl_bot_info *tglf_fetch_alloc_bot_info(struct tgl_state *TLS,
                                               struct tl_ds_bot_info *DS_BI) {
...
tglf_fetch_alloc_reply_markup(struct tgl_state *TLS, struct tgl_message *M,
...
void tgls_free_webpage(struct tgl_state *TLS, struct tgl_webpage *W) {
  ...
  if (W->display_url) {
    tfree_str(W->display_url);
  }
  if (W->title) {
    tfree_str(W->title);
  }
  if (W->site_name) {
    tfree_str(W->site_name);
  }
  if (W->type) {
    tfree_str(W->type);
  }
  if (W->description) {
    tfree_str(W->description);
  }
  if (W->photo) {
    tgls_free_photo(TLS, W->photo);
  }
  if (W->embed_url) {
    tfree_str(W->embed_url);
  }
  if (W->embed_type) {
    tfree_str(W->embed_type);
  }
  if (W->author) {
    tfree_str(W->author);
  }
...
void tgls_free_message_media(struct tgl_state *TLS,
                             struct tgl_message_media *M) {
  switch (M->type) {
  ...
  case tgl_message_media_venue:
    if (M->venue.title) {
      tfree_str(M->venue.title);
    }
    if (M->venue.address) {
      tfree_str(M->venue.address);
    }
    if (M->venue.provider) {
      tfree_str(M->venue.provider);
    }
    if (M->venue.venue_id) {
      tfree_str(M->venue.venue_id);
    }
    return;
  default:
...
void tgls_free_message_action(struct tgl_state *TLS,
                              struct tgl_message_action *M) {
  switch (M->type) {
...
void tgls_free_message_entity(struct tgl_state *TLS,
                              struct tgl_message_entity *E) {
  if (E->extra) {
    tfree_str(E->extra);
  }
...
void tgls_clear_message(struct tgl_state *TLS, struct tgl_message *M) {
  if (!(M->flags & TGLMF_SERVICE)) {
    if (M->message) {
      tfree(M->message, M->message_len + 1);
    }
...
void tgls_free_reply_markup(struct tgl_state *TLS,
                            struct tgl_message_reply_markup *R) {
  if (!--R->refcnt) {
    int i;
...
struct tgl_message *tglm_message_alloc(struct tgl_state *TLS,
                                       tgl_message_id_t *id) {
...
void tglm_message_insert_unsent(struct tgl_state *TLS, struct tgl_message *M) {
  TLS->message_unsent_tree =
      tree_insert_message(TLS->message_unsent_tree, M, rand());
}
...
void tglp_peer_insert_name(struct tgl_state *TLS, tgl_peer_t *P) {
  TLS->peer_by_name_tree =
      tree_insert_peer_by_name(TLS->peer_by_name_tree, P, rand());
}
...
struct tgl_message *tgl_message_get(struct tgl_state *TLS,
                                    tgl_message_id_t *msg_id) {
...
void tgl_peer_iterator_ex(struct tgl_state *TLS,
                          void (*it)(tgl_peer_t *P, void *extra), void *extra) {
  tree_act_ex_peer(TLS->peer_tree, it, extra);
}

int tgl_complete_user_list(struct tgl_state *TLS, int index, const char *text,
                           int len, char **R) {
  index++;
  while (index < TLS->peer_num &&
         (!TLS->Peers[index]->print_name ||
          strncmp(TLS->Peers[index]->print_name, text, len) ||
          tgl_get_peer_type(TLS->Peers[index]->id) != TGL_PEER_USER)) {
    index++;
  }
...
int tgl_complete_chat_list(struct tgl_state *TLS, int index, const char *text,
                           int len, char **R) {
  index++;
  while (index < TLS->peer_num &&
         (!TLS->Peers[index]->print_name ||
          strncmp(TLS->Peers[index]->print_name, text, len) ||
          tgl_get_peer_type(TLS->Peers[index]->id) != TGL_PEER_CHAT)) {
    index++;
  }
  if (index < TLS->peer_num) {
    *R = strdup(TLS->Peers[index]->print_name);
...
int tgl_complete_channel_list(struct tgl_state *TLS, int index,
                              const char *text, int len, char **R) {
  index++;
  while (index < TLS->peer_num &&
         (!TLS->Peers[index]->print_name ||
          strncmp(TLS->Peers[index]->print_name, text, len) ||
          tgl_get_peer_type(TLS->Peers[index]->id) != TGL_PEER_CHANNEL)) {
    index++;
  }
  if (index < TLS->peer_num) {
    *R = strdup(TLS->Peers[index]->print_name);
...
int tgl_complete_encr_chat_list(struct tgl_state *TLS, int index,
                                const char *text, int len, char **R) {
  index++;
  while (index < TLS->peer_num &&
         (!TLS->Peers[index]->print_name ||
          strncmp(TLS->Peers[index]->print_name, text, len) ||
          tgl_get_peer_type(TLS->Peers[index]->id) != TGL_PEER_ENCR_CHAT)) {
    index++;
  }
  if (index < TLS->peer_num) {
    *R = strdup(TLS->Peers[index]->print_name);
...
int tgl_complete_peer_list(struct tgl_state *TLS, int index, const char *text,
                           int len, char **R) {
  index++;
  while (index < TLS->peer_num &&
         (!TLS->Peers[index]->print_name ||
...
int tgl_secret_chat_for_user(struct tgl_state *TLS, tgl_peer_id_t user_id) {
  ...
  while (index < TLS->peer_num &&
         (tgl_get_peer_type(TLS->Peers[index]->id) != TGL_PEER_ENCR_CHAT ||
          TLS->Peers[index]->encr_chat.user_id != tgl_get_peer_id(user_id) ||
          TLS->Peers[index]->encr_chat.state != sc_ok)) {
    index++;
  }
  if (index < TLS->peer_num) {
...
void tgls_free_peer_gw(tgl_peer_t *P, void *TLS) { tgls_free_peer(TLS, P); }
...
int tgl_print_stat(struct tgl_state *TLS, char *s, int len) {
  return tsnprintf(s, len,
                   ...
                   TLS->users_allocated, TLS->chats_allocated,
                   TLS->encr_chats_allocated, TLS->peer_num,
                   TLS->messages_allocated);
}
...
void tgls_messages_mark_read(struct tgl_state *TLS, struct tgl_message *M,
                             int out, int seq) {
...
/*
void tgls_insert_random2local (struct tgl_state *TLS, long long random_id,
tgl_message_id_t *msg_id) { struct random2local *X = talloc (sizeof (*X));
...
tgl_message_id_t *tgls_get_local_by_random (struct tgl_state *TLS, long long
random_id) { struct random2local X; X.random_id = random_id; struct random2local
*Y = tree_lookup_random_id (TLS->random_id_tree, &X); if (Y) {
...
void tgls_insert_temp2local (struct tgl_state *TLS, int temp_id,
tgl_message_id_t *msg_id) { struct random2local *X = talloc (sizeof (*X));
  X->random_id = temp_id;
  X->local_id = *msg_id;
...
tgl_message_id_t *tgls_get_local_by_random(struct tgl_state *TLS,
                                           long long random_id) {
  struct tgl_message M;
...
tgl_message_id_t tgl_convert_temp_msg_id(struct tgl_state *TLS,
                                         tgl_message_id_t msg_id) {
  struct tgl_message M;
  M.temp_id = msg_id.id;
  struct tgl_message *N = tree_lookup_temp_id(TLS->temp_id_tree, &M);
...
void tgls_message_change_temp_id(struct tgl_state *TLS, struct tgl_message *M,
                                 int temp_id) {
  if (M->temp_id == temp_id) {
    return;
  }
  assert(!M->temp_id);
  M->temp_id = temp_id;
...
void tgls_message_change_random_id(struct tgl_state *TLS, struct tgl_message *M,
                                   long long random_id) {
