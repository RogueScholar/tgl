...
struct tgl_user *tglf_fetch_alloc_user (struct tgl_state *TLS,
    ...
    if (! U) {
        ...
        U->id = user_id;
        TLS->peer_tree =
            tree_insert_peer (TLS->peer_tree, (tgl_peer_t *)U, rand());
    ...
    if (new) {
        if (DS_EC->magic != CODE_encrypted_chat_requested) {
            vlogprintf (E_WARNING,
                        "Unknown chat. May be we forgot something...\n");
            return U;
        }
...
struct tgl_chat *tglf_fetch_alloc_chat (struct tgl_state *TLS,
    ...
    if (! C) {
        ...
        C->id = chat_id;
        TLS->peer_tree =
            tree_insert_peer (TLS->peer_tree, (tgl_peer_t *)C, rand());
        increase_peer_size (TLS);
...
tglf_fetch_alloc_chat_full (struct tgl_state *TLS,
    ...
    if (DS_CF->bot_info) {
        ...
        for (i = 0; i < n; i++) {
            ...
            tgl_peer_t *P =
                tgl_peer_get (TLS, TGL_MK_USER (DS_LVAL (DS_BI->user_id)));
            if (P && (P->flags & TGLCF_CREATED)) {
                bl_do_user (TLS,
                            tgl_get_peer_id (P->id),
                            NULL,
                            NULL,
                            0,
                            NULL,
...
void tglf_fetch_message_action (struct tgl_state *TLS,
    ...
    default:
        vlogprintf (E_WARNING,
                    "Unknown MessageAction code: magic = 0x%08x\n",
                    DS_MA->magic);
...
struct tgl_message *
tglf_fetch_alloc_message_short (struct tgl_state *TLS,
...
static int decrypt_encrypted_message (struct tgl_secret_chat *E) {
    ...
    TGLC_aes_ige_encrypt ((void *)decr_ptr,
                          (void *)decr_ptr,
                          4 * (decr_end - decr_ptr),
                          &aes_key,
                          iv,
...
tglf_fetch_encrypted_message (struct tgl_state *TLS,
    ...
    tgl_peer_t *P =
        tgl_peer_get (TLS, TGL_MK_ENCR_CHAT (DS_LVAL (DS_EM->chat_id)));
    ...
    if (M->flags & TGLMF_CREATED) {
        ...
        if (M->action.type == tgl_message_action_request_key) {
            if (E->exchange_state == tgl_sce_none
                ...
                tgl_do_accept_exchange (
                    TLS, E, M->action.exchange_id, M->action.g_a);
            } else {
                vlogprintf (
                    E_WARNING,
...
void tgl_peer_iterator_ex (struct tgl_state *TLS,
                           void (*it) (tgl_peer_t *P, void *extra),
                           void *extra) {
...
int tgl_complete_encr_chat_list (struct tgl_state *TLS, int index,
    ...
    while (index < TLS->peer_num
            && (! TLS->Peers[index]->print_name
                || strncmp (TLS->Peers[index]->print_name, text, len)
                || tgl_get_peer_type (TLS->Peers[index]->id)
                != TGL_PEER_ENCR_CHAT)) {
        index++;
    }
    if (index < TLS->peer_num) {
...
void tgls_message_change_random_id (struct tgl_state *TLS,
                                    struct tgl_message *M,
                                    long long random_id) {
