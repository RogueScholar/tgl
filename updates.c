...
int tgl_check_pts_diff (struct tgl_state *TLS, int pts, int pts_count) {
    ...
    if (pts > TLS->pts + pts_count) {
        vlogprintf (E_NOTICE,
                    "Hole in pts (pts = %d, count = %d, cur_pts = %d)\n",
                    pts,
                    pts_count,
                    TLS->pts);
...
int tgl_check_qts_diff (struct tgl_state *TLS, int qts, int qts_count) {
    ...
    if (qts > TLS->qts + qts_count) {
        vlogprintf (E_NOTICE,
                    "Hole in qts (qts = %d, count = %d, cur_qts = %d)\n",
                    qts,
                    qts_count,
                    TLS->qts);
        tgl_do_get_difference (TLS, 0, 0, 0);
...
void tglu_work_update (struct tgl_state *TLS, int check_only,
    ...
    if (DS_U->pts) {
        ...
        if (! check_only
                && tgl_check_pts_diff (
                    TLS, DS_LVAL (DS_U->pts), DS_LVAL (DS_U->pts_count))
                <= 0) {
            return;
        }
    }
    if (DS_U->qts) {
        if (! check_only
                && tgl_check_qts_diff (TLS, DS_LVAL (DS_U->qts), 1) <= 0) {
            return;
        }
    }
    if (DS_U->channel_pts) {
        ...
        if (! check_only
                && tgl_check_channel_pts_diff (TLS,
                                               E,
                                               DS_LVAL (DS_U->channel_pts),
                                               DS_LVAL (DS_U->channel_pts_count))
                <= 0) {
            return;
        }
    }
    ...
    case CODE_update_chat_user_typing:
    {
        ...
        if (U && C) {
            if (TLS->callback.type_in_chat_notification) {
                TLS->callback.type_in_chat_notification (
                    TLS, (void *)U, (void *)C, status);
            }
        }
    }
    break;
    case CODE_update_user_status:
    {
        ...
        if (U) {
            tglf_fetch_user_status (
                TLS, &U->user.status, &U->user, DS_U->status);
    ...
    case CODE_update_chat_participants:
    {
        ...
        if (C && (C->flags & TGLPF_CREATED)
            ...
            bl_do_chat (
                TLS,
                tgl_get_peer_id (chat_id),
                NULL,
                0,
                NULL,
                NULL,
                DS_U->participants->version,
                (struct tl_ds_vector *)DS_U->participants->participants,
                NULL,
                NULL,
                NULL,
                NULL,
    ...
    case CODE_update_new_authorization:
    {
        if (TLS->callback.new_authorization) {
            TLS->callback.new_authorization (
                TLS, DS_U->device->data, DS_U->location->data);
        }
    }
    break;
    ...
    case CODE_update_encrypted_chat_typing:
    {
        ...
        if (P) {
            if (TLS->callback.type_in_secret_chat_notification) {
                TLS->callback.type_in_secret_chat_notification (TLS,
                        (void *)P);
            }
        }
    }
    ...
    case CODE_update_chat_participant_add:
    {
        ...
        tgl_peer_t *C            = tgl_peer_get (TLS, chat_id);
        if (C && (C->flags & TGLPF_CREATED)) {
            bl_do_chat_add_user (TLS,
                                 C->id,
                                 version,
                                 tgl_get_peer_id (user_id),
                                 tgl_get_peer_id (inviter_id),
                                 time (0));
        }
    }
    break;
    case CODE_update_chat_participant_delete:
    {
        ...
        if (C && (C->flags & TGLPF_CREATED)) {
            bl_do_chat_del_user (
                TLS, C->id, version, tgl_get_peer_id (user_id));
        }
    }
    break;
    ...
    case CODE_update_user_blocked:
    {
        ...
        tgl_peer_t *P =
            tgl_peer_get (TLS, TGL_MK_USER (DS_LVAL (DS_U->user_id)));
    ...
    case CODE_update_service_notification:
    {
        ...
        if (TLS->callback.notification) {
            TLS->callback.notification (
                TLS, DS_U->type->data, DS_U->message_text->data);
        }
    }
    break;
    ...
    {
        tgl_do_get_channel_difference (
            TLS, DS_LVAL (DS_U->channel_id), NULL, NULL);
    }
    break;
