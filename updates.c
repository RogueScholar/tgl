...
void tgl_do_get_channel_difference(struct tgl_state *TLS, int channel_id,
                                   void (*callback)(struct tgl_state *TLS,
                                                    void *callback_extra,
                                                    int success),
                                   void *callback_extra);

static void fetch_dc_option(struct tgl_state *TLS,
                            struct tl_ds_dc_option *DS_DO) {
  vlogprintf(E_DEBUG, "id = %d, %.*s:%d\n", DS_LVAL(DS_DO->id),
             DS_RSTR(DS_DO->ip_address), DS_LVAL(DS_DO->port));
...
int tgl_check_channel_pts_diff(struct tgl_state *TLS, tgl_peer_t *_E, int pts,
                               int pts_count) {
  struct tgl_channel *E = &_E->channel;
  vlogprintf(E_DEBUG - 1,
             "channel %d: pts = %d, pts_count = %d, current_pts = %d\n",
...
void tglu_work_update(struct tgl_state *TLS, int check_only,
                      struct tl_ds_update *DS_U) {
  ...
  if (DS_U->pts) {
    ...
    if (!check_only && tgl_check_pts_diff(TLS, DS_LVAL(DS_U->pts),
                                          DS_LVAL(DS_U->pts_count)) <= 0) {
      return;
    }
  }
  if (DS_U->qts) {
  ...
  if (DS_U->channel_pts) {
    ...
    if (!check_only &&
        tgl_check_channel_pts_diff(TLS, E, DS_LVAL(DS_U->channel_pts),
                                   DS_LVAL(DS_U->channel_pts_count)) <= 0) {
      return;
    }
  }
  vlogprintf(E_NOTICE, "update 0x%08x (check=%d)\n", DS_U->magic, check_only);
  if (check_only > 0 && DS_U->magic != CODE_update_message_i_d) {
    return;
  }
  switch (DS_U->magic) {
  case CODE_update_new_message: {
    ...
    struct tgl_message *M =
        tglf_fetch_alloc_message(TLS, DS_U->message, &new_msg);
    assert(M);
    if (new_msg) {
  ...
  /*  case CODE_update_read_messages:
      {
        ...
        int i;
        for (i = 0; i < n; i++) {
          struct tgl_message *M = tgl_message_get (TLS, DS_LVAL
     (DS_U->messages->data[i])); if (M) { tgl_peer_t *P; if (M->flags &
     TGLMF_OUT) { P = tgl_peer_get (TLS, M->to_id); if (P && (P->flags &
     TGLMF_CREATED)) { if (tgl_get_peer_type (P->id) == TGL_PEER_USER) {
                  bl_do_user (TLS, tgl_get_peer_id (P->id), NULL, NULL, 0,
     NULL, 0, NULL, 0, NULL, 0, NULL, NULL, 0, NULL, 0, NULL, NULL, (int
     *)&M->id, TGL_FLAGS_UNCHANGED); } else { bl_do_chat (TLS,
     tgl_get_peer_id (P->id), NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, (int *)&M->id, TGL_FLAGS_UNCHANGED);
                }
              }
            } else {
              ...
              if (P && (P->flags & TGLMF_CREATED)) {
                if (tgl_get_peer_type (P->id) == TGL_PEER_USER) {
                  bl_do_user (TLS, tgl_get_peer_id (P->id), NULL, NULL, 0,
     NULL, 0, NULL, 0, NULL, 0, NULL, NULL, 0, NULL, 0, NULL, (int *)&M->id,
     NULL, TGL_FLAGS_UNCHANGED); } else { bl_do_chat (TLS, tgl_get_peer_id
     (P->id), NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, (int
     *)&M->id, NULL, TGL_FLAGS_UNCHANGED);
                }
              }
            }
          }
  ...
  case CODE_update_chat_user_typing: {
    ...
    if (U && C) {
      if (TLS->callback.type_in_chat_notification) {
        TLS->callback.type_in_chat_notification(TLS, (void *)U, (void *)C,
                                                status);
      }
    }
  } break;
  ...
  case CODE_update_user_name: {
    ...
    if (UC && (UC->flags & TGLPF_CREATED)) {
      bl_do_user(TLS, tgl_get_peer_id(user_id), NULL, DS_STR(DS_U->first_name),
                 DS_STR(DS_U->last_name), NULL, 0, DS_STR(DS_U->username), NULL,
                 NULL, NULL, NULL, NULL, TGL_FLAGS_UNCHANGED);
    }
  } break;
  case CODE_update_user_photo: {
    ...
    if (UC && (UC->flags & TGLUF_CREATED)) {
      bl_do_user(TLS, tgl_get_peer_id(user_id), NULL, NULL, 0, NULL, 0, NULL, 0,
                 NULL, 0, NULL, DS_U->photo, NULL, NULL, NULL,
                 TGL_FLAGS_UNCHANGED);
    }
  } break;
  ...
  case CODE_update_chat_participants: {
    ...
    if (C && (C->flags & TGLPF_CREATED) &&
        DS_U->participants->magic == CODE_chat_participants) {
      bl_do_chat(TLS, tgl_get_peer_id(chat_id), NULL, 0, NULL, NULL,
                 DS_U->participants->version,
                 (struct tl_ds_vector *)DS_U->participants->participants, NULL,
                 NULL, NULL, NULL, NULL, TGL_FLAGS_UNCHANGED);
    }
  } break;
  ...
  case CODE_update_new_authorization: {
    if (TLS->callback.new_authorization) {
      TLS->callback.new_authorization(TLS, DS_U->device->data,
                                      DS_U->location->data);
    }
  } break;
  ...
  case CODE_update_new_encrypted_message: {
    struct tgl_message *M =
        tglf_fetch_alloc_encrypted_message(TLS, DS_U->encr_message);
    if (M) {
      bl_do_msg_update(TLS, &M->permanent_id);
    }
  } break;
  case CODE_update_encryption: {
    struct tgl_secret_chat *E =
        tglf_fetch_alloc_encrypted_chat(TLS, DS_U->encr_chat);
    ...
    if (P && P->last) {
      ...
      while (M && (!(M->flags & TGLMF_OUT) || (M->flags & TGLMF_UNREAD))) {
        if (M->flags & TGLMF_OUT) {
          bl_do_edit_message_encr(TLS, &M->permanent_id, NULL, NULL, NULL, NULL,
                                  0, NULL, NULL, NULL,
  ...
  case CODE_update_chat_participant_add: {
    ...
    tgl_peer_t *C = tgl_peer_get(TLS, chat_id);
    if (C && (C->flags & TGLPF_CREATED)) {
      bl_do_chat_add_user(TLS, C->id, version, tgl_get_peer_id(user_id),
                          tgl_get_peer_id(inviter_id), time(0));
    }
  } break;
  ...
  case CODE_update_user_blocked: {
    ...
    if (P && (P->flags & TGLPF_CREATED)) {
      ...
      bl_do_user(TLS, tgl_get_peer_id(P->id), NULL, NULL, 0, NULL, 0, NULL, 0,
                 NULL, 0, NULL, NULL, NULL, NULL, NULL, flags);
    }
  } break;
  ...
  case CODE_update_service_notification: {
    vlogprintf(E_ERROR, "Notification %.*s: %.*s\n", DS_RSTR(DS_U->type),
               DS_RSTR(DS_U->message_text));
    if (TLS->callback.notification) {
      TLS->callback.notification(TLS, DS_U->type->data,
                                 DS_U->message_text->data);
    }
  } break;
  ...
  case CODE_update_user_phone: {
    ...
    if (U && (U->flags & TGLPF_CREATED)) {
      bl_do_user(TLS, tgl_get_peer_id(U->id), NULL, NULL, 0, NULL, 0,
                 DS_STR(DS_U->phone), NULL, 0, NULL, NULL, NULL, NULL, NULL,
                 TGL_FLAGS_UNCHANGED);
    }
  } break;
  case CODE_update_read_history_inbox: {
    ...
    if (P && (P->flags & TGLPF_CREATED)) {
      if (tgl_get_peer_type(P->id) == TGL_PEER_USER) {
        bl_do_user(TLS, tgl_get_peer_id(P->id), NULL, NULL, 0, NULL, 0, NULL, 0,
                   NULL, 0, NULL, NULL, DS_U->max_id, NULL, NULL,
                   TGL_FLAGS_UNCHANGED);
      } else {
        bl_do_chat(TLS, tgl_get_peer_id(P->id), NULL, 0, NULL, NULL, NULL, NULL,
                   NULL, NULL, NULL, DS_U->max_id, NULL, TGL_FLAGS_UNCHANGED);
      }
    }
  } break;
  case CODE_update_read_history_outbox: {
    ...
    if (P && (P->flags & TGLPF_CREATED)) {
      if (tgl_get_peer_type(P->id) == TGL_PEER_USER) {
        bl_do_user(TLS, tgl_get_peer_id(P->id), NULL, NULL, 0, NULL, 0, NULL, 0,
                   NULL, 0, NULL, NULL, NULL, DS_U->max_id, NULL,
                   TGL_FLAGS_UNCHANGED);
      } else {
        bl_do_chat(TLS, tgl_get_peer_id(P->id), NULL, 0, NULL, NULL, NULL, NULL,
                   NULL, NULL, NULL, NULL, DS_U->max_id, TGL_FLAGS_UNCHANGED);
      }
    }
  } break;
...
void tglu_work_updates(struct tgl_state *TLS, int check_only,
...
void tglu_work_updates_combined(struct tgl_state *TLS, int check_only,
                                struct tl_ds_updates *DS_U) {
  if (check_only > 0 || (TLS->locks & TGL_LOCK_DIFF)) {
    return;
  }
...
void tglu_work_update_short_message(struct tgl_state *TLS, int check_only,
                                    struct tl_ds_updates *DS_U) {
  if (check_only > 0 || (TLS->locks & TGL_LOCK_DIFF)) {
    return;
  }
...
void tglu_work_update_short_chat_message(struct tgl_state *TLS, int check_only,
                                         struct tl_ds_updates *DS_U) {
  if (check_only > 0 || (TLS->locks & TGL_LOCK_DIFF)) {
    return;
  }
...
void tglu_work_updates_too_long(struct tgl_state *TLS, int check_only,
                                struct tl_ds_updates *DS_U) {
  if (check_only > 0 || (TLS->locks & TGL_LOCK_DIFF)) {
    return;
  }
...
void tglu_work_update_short(struct tgl_state *TLS, int check_only,
                            struct tl_ds_updates *DS_U) {
  if (check_only > 0 || (TLS->locks & TGL_LOCK_DIFF)) {
    return;
  }
...
void tglu_work_update_short_sent_message(struct tgl_state *TLS, int check_only,
                                         struct tl_ds_updates *DS_U,
                                         void *extra) {
  if (DS_U->pts) {
    assert(DS_U->pts_count);
  ...
  bl_do_edit_message(TLS, &M->permanent_id, NULL, NULL, NULL, NULL, NULL, NULL,
                     0, DS_U->media, NULL, NULL, NULL, NULL, flags);
  if (check_only) {
    return;
  }
  bl_do_msg_update(TLS, &M->permanent_id);
...
void tglu_work_any_updates(struct tgl_state *TLS, int check_only,
                           struct tl_ds_updates *DS_U, void *extra) {
  if (check_only > 0 || (TLS->locks & TGL_LOCK_DIFF)) {
    return;
  }
  switch (DS_U->magic) {
...
void tgl_insert_status_update(struct tgl_state *TLS, struct tgl_user *U) {
  ...
  if (!TLS->online_updates_timer) {
    TLS->online_updates_timer =
        TLS->timer_methods->alloc(TLS, status_notify, 0);
...
void tgl_insert_status_expire(struct tgl_state *TLS, struct tgl_user *U) {
  ...
  TLS->timer_methods->insert(U->status.ev,
                             U->status.when - tglt_get_double_time());
}
