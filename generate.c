...
int is_empty(struct tl_type *t) {
    if (t->name == NAME_INT || t->name == NAME_LONG || t->name == NAME_DOUBLE ||
            t->name == NAME_STRING) {
        return 1;
    }
...
struct tl_tree_methods tl_var_num_methods = {.type = tl_tree_type_var_num};

struct tl_tree_methods tl_var_type_methods = {.type = tl_tree_type_var_type};

struct tl_tree_methods tl_type_methods = {.type = tl_tree_type_type};

struct tl_tree_methods tl_nat_const_methods = {.type = tl_tree_type_nat_const};

struct tl_tree_methods tl_array_methods = {.type = tl_tree_type_array};

struct tl_tree_methods tl_ptype_methods = {.type = tl_tree_type_type};

struct tl_tree_methods tl_parray_methods = {.type = tl_tree_type_array};

struct tl_tree_methods tl_pvar_num_methods = {.type = tl_tree_type_var_num};

struct tl_tree_methods tl_pvar_type_methods = {.type = tl_tree_type_var_type};

struct tl_tree_methods tl_nat_const_full_methods = {.type =
        tl_tree_type_nat_const
};
...
int gen_uni_skip(struct tl_tree *t, char *cur_name, int *vars, int first,
    ...
    case NODE_TYPE_TYPE:
        ...
        if (!first) {
            printf("  if (ODDP(%s) || %s->type->name != 0x%08x) { %s }\n", cur_name,
                   cur_name, t1->type->name, fail);
        } else {
            printf("  if (ODDP(%s) || (%s->type->name != 0x%08x && %s->type->name != "
                   "0x%08x)) { %s }\n",
    ...
    case NODE_TYPE_NAT_CONST:
        printf("  if (EVENP(%s) || ((long)%s) != %" INT64_PRINTF_MODIFIER
               "d) { %s }\n",
...
int gen_create(struct tl_tree *t, int *vars, int offset) {
    ...
    case NODE_TYPE_TYPE:
        ...
        if (t1->self.flags & FLAG_BARE) {
            printf(
                ".type = &(struct tl_type_descr) {.name = 0x%08x, .id = \"Bare_%s\", "
                ".params_num = %d, .params_types = %" INT64_PRINTF_MODIFIER "d},\n",
                ~t1->type->name, t1->type->id, t1->type->arity,
                t1->type->params_types);
        } else {
            printf(".type = &(struct tl_type_descr) {.name = 0x%08x, .id = \"%s\", "
                   ".params_num = %d, .params_types = %" INT64_PRINTF_MODIFIER
                   "d},\n",
                   t1->type->name, t1->type->id, t1->type->arity,
                   t1->type->params_types);
        }
    ...
    case NODE_TYPE_ARRAY:
        ...
        print_offset(offset + 2);
        t2 = (void *)t;
        printf(".type = &(struct tl_type_descr) {.name = NAME_ARRAY, .id = "
               "\"array\", .params_num = 2, .params_types = 1},\n");
        print_offset(offset + 2);
        printf(".params = (struct paramed_type **){\n");
    ...
    case NODE_TYPE_VAR_NUM:
        print_offset(offset);
        printf("((void *)var%d) + %d", ((struct tl_tree_var_type *)t)->var_num,
               2 * ((struct tl_tree_var_num *)t)->dif);
        return 0;
    default:
        assert(0);
        return -1;
        ...
        if (t == NODE_TYPE_TYPE || t == NODE_TYPE_VAR_TYPE) {
            ...
            if (!bare) {
                printf("%sif (skip_type_%s (field%d) < 0) { return -1;}\n", offset,
                       t == NODE_TYPE_VAR_TYPE
                       ? "any"
                       : ((struct tl_tree_type *)arg->type)->type->print_id,
                       num);
            } else {
            ...
            assert(gen_create(((struct tl_tree_array *)arg->type)->multiplicity, vars,
                              2 + o) >= 0);
            printf("%s);\n", offset);
            printf("%sstruct paramed_type *field%d = \n", offset, num);
            assert(gen_create(((struct tl_tree_array *)arg->type)->args[0]->type,
                              vars, 2 + o) >= 0);
            printf(";\n");
...
int gen_field_fetch(struct arg *arg, int *vars, int num, int empty) {
    ...
    if (arg->var_num >= 0) {
        ...
        if (t == NODE_TYPE_TYPE || t == NODE_TYPE_VAR_TYPE) {
            ...
            printf("%sif (multiline_output >= 1) { multiline_offset += "
                   "multiline_offset_size;}\n",
                   offset);
            printf("%swhile (multiplicity%d -- > 0) {\n", offset, num);
            ...
            printf("%s}\n", offset);
            printf("%sif (multiline_output >= 1) { multiline_offset -= "
                   "multiline_offset_size; print_offset ();}\n",
                   offset);
            printf("%seprintf (\" ]\");\n", offset);
        }
    }
...
int gen_field_store(struct arg *arg, int *vars, int num, int from_func,
    ...
    if (arg->id && strlen(arg->id) > 0 && !empty) {
        printf(
            "%sif (cur_token_len >= 0 && cur_token_len == %d && !cur_token_quoted "
            "&& !memcmp (cur_token, \"%s\", cur_token_len)) {\n",
...
int gen_field_autocomplete(struct arg *arg, int *vars, int num, int from_func,
    ...
    char *fail = from_func ? "0" : "-1";
    char *expect =
        from_func ? "expect_token_ptr_autocomplete" : "expect_token_autocomplete";
    if (arg->id && strlen(arg->id) > 0 && !empty) {
        printf("%sif (cur_token_len == -3 && cur_token_real_len <= %d && "
               "!cur_token_quoted && !memcmp (cur_token, \"%s\", "
               "cur_token_real_len)) {\n",
               offset, (int)(strlen(arg->id)), arg->id);
        ...
        printf("%sif (cur_token_len >= 0 && cur_token_len == %d && !memcmp "
               "(cur_token, \"%s\", cur_token_len)) {\n",
               offset, (int)(strlen(arg->id)), arg->id);
        printf("%s  local_next_token ();\n", offset);
        printf("%s  %s (\":\", 1);\n", offset, expect);
        printf("%s}\n", offset);
...
int gen_field_fetch_ds(struct arg *arg, int *vars, int num, int empty) {
    ...
    if (arg->var_num >= 0) {
        ...
        if (t == NODE_TYPE_TYPE || t == NODE_TYPE_VAR_TYPE) {
            ...
            if (t == NODE_TYPE_TYPE &&
                    !strcmp(((struct tl_tree_type *)arg->type)->type->id, "Vector")) {
                printf("(void *)");
            }
            if (!bare) {
    ...
    if (arg->var_num >= 0) {
        ...
        if (t == NODE_TYPE_TYPE || t == NODE_TYPE_VAR_TYPE) {
            ...
            int any = (t == NODE_TYPE_VAR_TYPE) ||
                      ((struct tl_tree_type *)arg->type)->type->name == NAME_VECTOR;
            if (arg->id && strlen(arg->id)) {
                printf("%sfree_ds_type_%s (D->%s, field%d);\n", offset,
                       any ? "any" : ((struct tl_tree_type *)arg->type)->type->print_id,
                       arg->id, num);
            } else {
...
int gen_field_autocomplete_excl(struct arg *arg, int *vars, int num,
                                int from_func) {
    assert(arg);
...
void gen_type_fetch(struct tl_type *t) {
    ...
    printf("  if (in_remaining () < 4) { return -1;}\n");
    if (!empty) {
        printf("  if (multiline_output >= 2) { multiline_offset += "
               "multiline_offset_size; }\n");
        printf("  eprintf (\" (\");\n");
    }
    printf("  int magic = fetch_int ();\n");
    ...
    if (t->constructors_num > 1) {
        ...
        for (i = 0; i < t->constructors_num; i++) {
            printf(
                "  if (skip_constructor_%s (T) >= 0) { in_ptr = save_in_ptr; "
                "%sassert (!fetch_constructor_%s (T)); %sreturn 0; }\n",
                t->constructors[i]->print_id, empty ? "" : "eprintf (\" (\"); ",
                t->constructors[i]->print_id,
                empty
                ? ""
                : "if (multiline_output >= 2) { multiline_offset -= "
                "multiline_offset_size; print_offset (); } eprintf (\" )\");");
            printf("  in_ptr = save_in_ptr;\n");
        }
    } else {
        for (i = 0; i < t->constructors_num; i++) {
            ...
            printf(
                "  if (fetch_constructor_%s (T) >= 0) { %sreturn 0; }\n",
                t->constructors[i]->print_id,
                empty
                ? ""
                : "if (multiline_output >= 2) { multiline_offset -= "
...
void gen_type_store(struct tl_type *t) {
    ...
    for (k = 0; k < 2; k++) {
        ...
        if (empty) {
            ...
            int i;
            for (i = 0; i < t->constructors_num; i++) {
                printf("  if (cur_token_len == %d && !memcmp (cur_token, \"%s\", "
                       "cur_token_len)) {\n",
...
void gen_type_autocomplete(struct tl_type *t) {
    ...
    for (k = 0; k < 2; k++) {
        ...
        if (empty) {
            ...
            printf("  if (cur_token_len == -3) { set_autocomplete_type "
                   "(do_autocomplete_type_%s); return -1; }\n",
                   t->print_id);
            printf("  if (cur_token_len < 0) { return -1; }\n");
            int i;
            for (i = 0; i < t->constructors_num; i++) {
            ...
            /*if (t->constructors_num == 1) {
              printf ("  if (!force) {\n");
              printf ("    if (autocomplete_constructor_%s (T) < 0) { return -1; }\n",
            t->constructors[0]->print_id); printf ("    expect_token_autocomplete
            (\")\", 1);\n"); printf ("    return 0;\n"); printf ("  }\n");
            }*/
            printf("  return -1;\n");
            printf("}\n");
...
void gen_type_fetch_ds(struct tl_type *t) {
    ...
    if (t->constructors_num > 1) {
        ...
        for (i = 0; i < t->constructors_num; i++) {
            printf("  if (skip_constructor_%s (T) >= 0) { in_ptr = save_in_ptr; "
                   "return fetch_ds_constructor_%s (T); }\n",
...
void gen_function_store(struct tl_combinator *f) {
    ...
    for (i = 0; i < f->args_num; i++)
        if (!(f->args[i]->flags & FLAG_OPT_VAR)) {
...
void gen_type_do_autocomplete(struct tl_type *t) {
    printf("int do_autocomplete_type_%s (const char *text, int text_len, int "
           "index, char **R) {\n",
           t->print_id);
    printf("  index ++;\n");
    int i;
    for (i = 0; i < t->constructors_num; i++) {
        printf("  if (index == %d) { if (!strncmp (text, \"%s\", text_len)) { *R = "
               "tstrdup (\"%s\"); return index; } else { index ++; }}\n",
...
struct tl_tree *read_type(int *var_num) {
    ...
    for (i = 0; i < T->children_num; i++) {
        ...
        if (!TL_IS_NAT_VAR(T->children[i]) &&
                !(T->children[i]->flags & FLAG_NOVAR)) {
            T->self.flags &= ~FLAG_NOVAR;
        }
    }
    return (void *)T;
...
int read_args_list(struct arg **args, int args_num, int *var_num) {
    ...
    for (i = 0; i < args_num; i++) {
        ...
        if (args[i]->var_num < 0 && args[i]->exist_var_num < 0 &&
                (TL_IS_NAT_VAR(args[i]->type) || (args[i]->type->flags & FLAG_NOVAR))) {
...
void gen_skip_header(void) {
    ...
    for (i = 0; i < tn; i++)
        if (tps[i]->id[0] != '#' && strcmp(tps[i]->id, "Type")) {
            printf("int skip_type_%s (struct paramed_type *T);\n", tps[i]->print_id);
            printf("int skip_type_bare_%s (struct paramed_type *T);\n",
                   tps[i]->print_id);
        }
...
void gen_skip_source(void) {
    ...
    for (i = 0; i < tn; i++)
        if (tps[i]->id[0] != '#' && strcmp(tps[i]->id, "Type") && tps[i]->name) {
...
void gen_fetch_header(void) {
    ...
    printf("char *tglf_extf_fetch (struct tgl_state *TLS, struct paramed_type "
           "*T);\n");
...
void gen_store_header(void) {
    ...
    printf("struct paramed_type *tglf_extf_store (struct tgl_state *TLS, const "
           "char *data, int data_len);\n");
    printf("int tglf_store_type (struct tgl_state *TLS, const char *work, int "
           "work_len, struct paramed_type *P);\n");
...
void gen_autocomplete_header(void) {
    ...
    printf("int tglf_extf_autocomplete (struct tgl_state *TLS, const char *text, "
           "int text_len, int index, char **R, char *data, int data_len);\n");
    ...
    for (i = 0; i < tn; i++)
        if (tps[i]->id[0] != '#' && strcmp(tps[i]->id, "Type")) {
            printf("int autocomplete_type_%s (struct paramed_type *T);\n",
                   tps[i]->print_id);
            printf("int do_autocomplete_type_%s (const char *text, int len, int "
                   "index, char **R);\n",
                   tps[i]->print_id);
...
void gen_autocomplete_source(void) {
    ...
    printf("  default: return -1; }\n");
    printf("}\n");
    printf("int do_autocomplete_function (const char *text, int text_len, int "
           "index, char **R) {\n");
    printf("  index ++;\n");
    for (i = 0; i < fn; i++) {
        printf("  if (index == %d) { if (!strncmp (text, \"%s\", text_len)) { *R = "
        ...
        if (tps[i]->id[0] != '#' && strcmp(tps[i]->id, "Type")) {
            ...
            for (j = 0; j < tps[i]->constructors_num; j++) {
                ...
                for (k = 0; k < c->args_num; k++) {
                    if ((c->args[k]->flags & FLAG_OPT_VAR)) {
                        continue;
                    }
...
void gen_free_ds_header(void) {
    ...
    for (i = 0; i < tn; i++) {
        for (j = 0; j < tps[i]->constructors_num; j++) {
            printf("void free_ds_constructor_%s (",
                   tps[i]->constructors[j]->print_id);
            print_c_type_name(tps[i]->constructors[j]->result, "", 0);
            printf("D, struct paramed_type *T);\n");
        }
...
void gen_store_ds_header(void) {
    ...
    for (i = 0; i < tn; i++) {
        for (j = 0; j < tps[i]->constructors_num; j++) {
            printf("void store_ds_constructor_%s (",
                   tps[i]->constructors[j]->print_id);
            print_c_type_name(tps[i]->constructors[j]->result, "", 0);
            printf("D, struct paramed_type *T);\n");
        }
...
void gen_print_ds_header(void) {
    ...
    printf("struct tgl_state;\n");
    printf("char *tglf_extf_print_ds (struct tgl_state *TLS, void *DS, struct "
           "paramed_type *T);\n");

    int i, j;
    for (i = 0; i < tn; i++) {
        for (j = 0; j < tps[i]->constructors_num; j++) {
            printf("int print_ds_constructor_%s (",
                   tps[i]->constructors[j]->print_id);
            print_c_type_name(tps[i]->constructors[j]->result, "", 0);
            printf("DS, struct paramed_type *T);\n");
        }
